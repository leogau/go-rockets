...
import (
	...
	"leogau.dev/rockets-go-tutorial/seam"
	"leogau.dev/rockets-go-tutorial/unsplash"
)

const IMAGE_URL string = "https://bit.ly/2QGPDkr"
...
func worker(id int, taskChannel <-chan Task, resultChannel chan<- TaskResult) {
	for j := range taskChannel {
		fmt.Println("worker", id, "stared job", j.Position)
